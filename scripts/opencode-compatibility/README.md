...
The report also records and explicitly probes the OpenCode version declared by `@opencode-ai/sdk` in Varro's `package.json`. A successful floor check updates the tracked `verified.json` summary, and unit tests reject a manifest ceiling change until that release has passed the real Docker probe. Background updates do not cross the manifest version. The update checker ignores available releases above that ceiling unless the debug-only update-suggestion setting is enabled; an already installed newer server is still allowed to run.
