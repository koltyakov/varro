...
export function Chat() {
  ...
  return (
    <div class="flex h-full min-h-0 flex-col">
      <div class="flex items-center justify-between border-b border-vscode-border/40 px-4 py-2.5">
        <button
          class="flex min-w-0 flex-1 items-center gap-2.5 rounded-md px-2 py-1.5 text-[13px] font-medium transition-colors hover:bg-vscode-hover"
        ...
        >
          ...
          <div class="min-w-0 flex-1 text-left">
            <div class="truncate" title={activeTitle()}>
              {activeTitle()}
            </div>
          </div>
          <svg
            class={`h-3.5 w-3.5 shrink-0 text-vscode-muted/50 transition-transform ${showSessionPicker() ? "rotate-180" : ""}`}
            viewBox="0 0 16 16"
            fill="currentColor"
          >
        ...
        <div class="ml-1.5 flex shrink-0 items-center gap-0.5">
          <button
            class="rounded-md p-2 text-vscode-muted transition-colors hover:bg-vscode-hover hover:text-vscode-fg"
...
function SessionOverlay() {
  return (
    <div class="max-h-75 overflow-y-auto border-b border-vscode-border/40 bg-vscode-card/60 animate-fade-in">
      <Show
        when={state.sessions.length > 0}
        fallback={
          <div class="px-5 py-10 text-center text-[13px] text-vscode-muted">No previous sessions</div>
        }
      >
        <For each={state.sessions}>
          {(session) => (
            <div
              class={`group flex w-full cursor-pointer items-center justify-between gap-3 border-b border-vscode-border/20 px-4 py-4 transition-colors last:border-b-0 hover:bg-vscode-hover ${
            ...
            >
              <div class="min-w-0 flex-1">
                <div class="truncate text-[13px] font-medium text-vscode-fg">{session.title || "Untitled"}</div>
                <Show when={session.summary}>
                  <div class="mt-1.5 text-[12px] text-vscode-muted">
              ...
              <button
                class="shrink-0 rounded-md p-2 text-vscode-muted opacity-0 transition-all hover:bg-vscode-error/10 hover:text-vscode-error group-hover:opacity-100"
                onClick={(e) => {
                  e.stopPropagation()
