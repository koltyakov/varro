export function BusySendMenu(props: {
  ref?: HTMLDivElement | ((el: HTMLDivElement) => void);
  onQueue: () => void;
  onSteer: () => void;
  onStopAndSend: () => void;
}) {
  return (
    <div ref={props.ref} class="toolbar-popover busy-menu" onClick={(e) => e.stopPropagation()}>
      <button class="toolbar-popover-item" onClick={props.onQueue}>
        <span class="busy-menu-icon">
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
            aria-hidden="true"
          >
            <path d="M18 22H6C4.34315 22 3 20.6569 3 19L3 17C3 15.3431 4.34315 14 6 14H18C19.6569 14 21 15.3431 21 17V19C21 20.6569 19.6569 22 18 22Z" />
            <path
              d="M3 8V6C3 3.79086 4.79086 2 7 2L17 2C19.2091 2 21 3.79086 21 6V8"
              stroke-linecap="round"
              stroke-linejoin="round"
            />
            <path d="M12 14V6M12 6L9 9M12 6L15 9" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </span>
        <span class="busy-menu-label">Add to Queue</span>
        <span class="busy-menu-hint">Enter</span>
      </button>
      <button class="toolbar-popover-item" onClick={props.onSteer}>
        <span class="busy-menu-icon">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M2.25 17C2.25 14.9289 3.92894 13.25 6 13.25H18C20.0711 13.25 21.75 14.9289 21.75 17V19C21.75 21.0711 20.0711 22.75 18 22.75H6C3.92894 22.75 2.25 21.0711 2.25 19V17Z"
            />
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M7 2.75C5.20507 2.75 3.75 4.20507 3.75 6V8C3.75 8.41421 3.41421 8.75 3 8.75C2.58579 8.75 2.25 8.41421 2.25 8V6C2.25 3.37665 4.37665 1.25 7 1.25H17C19.6233 1.25 21.75 3.37664 21.75 6V8C21.75 8.41421 21.4142 8.75 21 8.75C20.5858 8.75 20.25 8.41421 20.25 8V6C20.25 4.20508 18.7949 2.75 17 2.75H7Z"
            />
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M11.4697 5.46967C11.7626 5.17678 12.2374 5.17678 12.5303 5.46967L15.5303 8.46967C15.8232 8.76256 15.8232 9.23744 15.5303 9.53033C15.2374 9.82322 14.7626 9.82322 14.4697 9.53033L12.75 7.81066V14C12.75 14.4142 12.4142 14.75 12 14.75C11.5858 14.75 11.25 14.4142 11.25 14V7.81066L9.53033 9.53033C9.23744 9.82322 8.76256 9.82322 8.46967 9.53033C8.17678 9.23744 8.17678 8.76256 8.46967 8.46967L11.4697 5.46967Z"
            />
          </svg>
        </span>
        <span class="busy-menu-label">Steer with Message</span>
        <span class="busy-menu-hint">{'\u2303'}Enter</span>
      </button>
      <button class="toolbar-popover-item" onClick={props.onStopAndSend}>
        <span class="busy-menu-icon" style={{ color: 'var(--color-vscode-error)' }}>
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
            aria-hidden="true"
          >
            <path d="M6.75827 17.2426L12.0009 12M17.2435 6.75736L12.0009 12M12.0009 12L6.75827 6.75736M12.0009 12L17.2435 17.2426" />
          </svg>
        </span>
        <span class="busy-menu-label">Stop and Send</span>
      </button>
    </div>
  );
}
