...
export function ToolCall(props: { part: ToolPart }) {
  ...
  return (
    <div class={`my-2 border-l-2 transition-colors ${
      state().status === "running"
        ? "border-l-vscode-accent bg-vscode-accent/5"
        : state().status === "error"
          ? "border-l-vscode-error bg-vscode-error/5"
          : "border-l-vscode-border bg-transparent"
    }`}>
      <button
        class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-[12px] transition-colors hover:bg-vscode-hover/50"
        onClick={() => setExpanded(!expanded())}
      >
        <span class={`h-1.5 w-1.5 shrink-0 ${statusDot()}`} />
        ...
        <svg
          class={`h-3 w-3 shrink-0 text-vscode-muted/50 transition-transform ${expanded() ? "rotate-90" : ""}`}
          viewBox="0 0 16 16"
          fill="currentColor"
        >
      ...
      <Show when={expanded()}>
        <div class="border-t border-vscode-border/30 px-3 py-2 text-[11px] animate-fade-in">
          <Show when={Object.keys(state().input || {}).length > 0}>
            <div class="mb-3">
              ...
              <pre class="overflow-x-auto whitespace-pre-wrap bg-vscode-input-bg/60 p-2 font-mono text-[11px] text-vscode-fg">
          ...
          <Show when={state().status === "completed"}>
            <div>
              ...
              <pre class="max-h-[200px] overflow-auto whitespace-pre-wrap bg-vscode-input-bg/60 p-2 font-mono text-[11px] text-vscode-fg">
                {(state() as any).output || "(empty)"}
              </pre>
            </div>
          </Show>
          <Show when={state().status === "error"}>
            <div class="bg-vscode-error/10 p-2 text-vscode-error">
              {(state() as any).error}
            </div>
          </Show>
