/* oxlint-disable anti-slop/require-safety-comment-for-type-assertion -- SAFETY: This E2E check inspects controlled access-mode state exposed by the harness browser global. */
import { expect, test } from '@playwright/test';
import { getE2EState } from './helpers';

test('full access mode sends a bash request without showing a permission prompt', async ({
  page,
}) => {
  ...
  await composer.fill(
    'In full access mode, get opencode version using bash by running opencode --version.'
  );
