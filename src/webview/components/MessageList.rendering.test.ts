...
describe('MessageList compact activity', () => {
  ...
  it.each([false, true])(
    'groups loaded instructions with adjacent tools, instructions first=%s',
    async (instructionsFirst) => {
      const command = toolPart('command-1', 'assistant-1', 'call-command-1');
      command.state = {
        status: 'completed',
        input: { command: 'npm test' },
      ...
      const instructions: MessageEntry = {
        info: userMessage('instructions'),
        parts: [
          {
            id: 'rules',
            sessionID: 'session-1',
      ...
      replaceMessages([
        { info: userMessage('user-1'), parts: [textPart('prompt-1', 'Run the checks')] },
        ...(instructionsFirst ? [instructions, activity] : [activity, instructions]),
        {
          info: assistantMessage('assistant-2', { parentID: 'user-1' }),
          parts: [
            {
      ...
      expect(summaries()).toHaveLength(1);
      expect(summaries()[0]?.textContent).toContain('1 command, 1 tool call');
      expect(container?.querySelector('.automated-message')).toBeNull();
      const hiddenId = instructionsFirst ? 'assistant-1' : 'instructions';
      expect(container?.querySelector(`[data-msg-id="${hiddenId}"]`)?.classList).toContain(
        'interactive-item-render-empty'
      );
      summaries()[0]?.click();
      expect(container?.textContent).toContain('Loaded agent instructions: /repo/AGENTS.md');
      expect(container?.querySelector(`[data-msg-id="${hiddenId}"]`)?.classList).not.toContain(
        'interactive-item-render-empty'
      );
      expect(
        container?.querySelector('[data-msg-id="instructions"] .user-message-card')
      ).toBeNull();
      summaries()[0]?.click();
      ...
      expect(
        state.messages.find((message) => message.info.id === 'instructions')?.parts[0]?.type
      ).toBe('text');
    }
  );
