...
- `varro.chat.sessionHistoryScope` - choose which OpenCode sessions appear for a VS Code workspace folder. `directory` shows exact working-directory matches and is the default, `descendants` also shows sessions below the folder within the same OpenCode project, and `project` shows the entire containing Git project. This is a resource-scoped setting, so each folder in a multi-root workspace can use a different value. For folders outside Git, `project` safely behaves as `descendants` instead of exposing OpenCode's global non-Git project. Nested folders that belong to a different OpenCode project are not included by `descendants`
