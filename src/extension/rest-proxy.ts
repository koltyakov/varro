...
export { resolveOpenCodeProjectConfigPaths } from './opencode-config-service';
...
export class RestProxy {
  ...
  private async handleRequestInScope(payload: ApiRequestPayload) {
    ...
    try {
      ...
      if (method === 'POST' && payload.path === VARRO_API_ENDPOINTS.openCodeConfigDisableProvider) {
        ...
        await this.openCodeConfig.disableOpenCodeProvider(providerID);
      ...
      if (permissionConfigRequest) {
        const data =
          permissionConfigRequest.kind === 'get'
            ? await this.openCodeConfig.readOpenCodePermissionConfig()
            : await this.openCodeConfig.updateOpenCodePermissionConfig(
                permissionConfigRequest.rules
              );
        this.callbacks.postApiResponse(requestGeneration, { id: payload.id, data });
        return;
      }
      ...
      if (sessionPermissionRulesRequest) {
        ...
        if (sessionPermissionRulesRequest.kind === 'get') {
          ...
          const data = this.openCodeConfig.normalizeSessionPermissionRules(session?.permission);
          this.callbacks.postApiResponse(requestGeneration, { id: payload.id, data });
          return;
        }
      ...
      if (serverMemoryRequest) {
        ...
        const data = await this.openCodeConfig.readServerMemoryPermissions(
      ...
      const legacyServerMemoryRules = await this.openCodeConfig.prepareLegacyServerMemoryRules(
        method,
        payload.path,
        payload.body,
        explicitWorkspaceDirectory ?? this.getCurrentWorkspaceResolutionRoot()
      ...
      try {
        ...
        if (this.isSessionListRequest(method, payload.path) && sessionPageLimit === null) {
          void this.trackSessionDirectoryBootstrap(responsePromise, false);
        }
      } catch (err) {
  ...
  private async persistProjectPermissionAllow(
    ...
    await this.openCodeConfig.updateOpenCodeProjectPermission(permission, patterns);
  ...
  private async requestSessionMessagesForSummary(path: string) {
    try {
      return sessionSummary.projectHistory(
      ...
      return sessionSummary.omittedHistory();
    }
  }
