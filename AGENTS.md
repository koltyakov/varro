...
- For AI verification, start with `docs/ai-test-workflow.md`. Own setup and bounded recovery, use retained playback for rendering reproduction, continue independent scenarios after a missed precondition, and record additional AI-directed investigations. Ask the user only for prerequisites that actually require their decision.
...
- AI parity and regression runs must include `docs/ai-action-matrix.md`: test sends, queueing, pause/edit/resume/remove, steering, inline edits, tools, actual subagents, root/child permissions, questions, cancellation, and view ownership recovery. Report action-by-backend results; streaming-only coverage is incomplete.
- When the user asks to "Run streaming tests" (case-insensitive), follow `docs/ai-streaming-verification.md`. This separate AI-test suite selects existing OpenCode history read-only and replays it in an isolated VS Code host with short gaps preserved and long waits capped. Review selection coverage, attach AI watchers before starting playback, save evidence under `artifacts/ai-streaming/`, and never treat scheduler/canonical checks alone as a visual or performance pass. Never execute recorded tools or mutate source sessions.
- Tests must never change existing production sessions, including timestamps, permissions, titles, or messages, unless the user explicitly requests that specific change. A test request is not authorization to migrate or repair production data. Live AI tests require a verified separate database under `artifacts/ai-test-data/`; never fall back to the normal server, reuse a production editor, or bypass isolation checks. Historical session reads remain read-only; test imported copies instead.
