...
export function Message(props: { info: MessageType; parts: Part[]; isFirstInGroup?: boolean }) {
  ...
  return (
    <article class={props.isFirstInGroup !== false ? (isUser() ? "mt-4 first:mt-0" : "mt-1") : "mt-0.5"}>
      <Show when={props.isFirstInGroup !== false}>
        <div class={`mb-0.5 text-[11px] font-medium ${isUser() ? "text-vscode-fg" : "text-vscode-muted"}`}>
...
function DiffSummary(props: { diffs: FileDiff[] }) {
  ...
  return (
    <div class="mt-1">
      ...
      >
        <svg class={`h-3 w-3 transition-transform ${expanded() ? "rotate-90" : ""}`} viewBox="0 0 16 16" fill="currentColor">
