...
const executable =
  process.env.VARRO_VSCODE_EXECUTABLE || '/Applications/Visual Studio Code.app/Contents/MacOS/Code';
...
const provider = createServer(async (request, response) => {
  ...
  if (body.model === 'unauthorized') {
    ...
    response.end(
      JSON.stringify({
        error: {
          message: 'UI fixture authentication rejected',
    ...
    response.end(
      JSON.stringify({
        ...
        choices: [
          {
            index: 0,
            message: { role: 'assistant', content: 'UI adapter reply.' },
            finish_reason: 'stop',
          },
        ],
  ...
  for (const content of ['UI ', 'adapter ', 'reply.']) {
    response.write(
      `data: ${JSON.stringify({ id: 'completion-ui', object: 'chat.completion.chunk', created: 1, model: 'fixture', choices: [{ index: 0, delta: { content }, finish_reason: null }] })}\n\n`
    );
    await delay(100);
  }
  response.end(
...
await writeFile(
  ...
  JSON.stringify({
    ...
    agents: { title: { model: 'fixture/fixture' } },
    provider: {
      fixture: {
        npm: '@ai-sdk/openai-compatible',
        name: 'UI fixture',
        options: { baseURL: modelUrl, apiKey: 'fixture-only' },
        models: {
          fixture: { name: 'UI fixture model', limit: { context: 32000, output: 1000 } },
          unauthorized: {
            name: 'UI authorization failure',
      ...
      recovery: {
        npm: '@ai-sdk/openai-compatible',
        name: 'UI recovery fixture',
        options: { baseURL: modelUrl, apiKey: 'fixture-only' },
        models: {
          recovery: { name: 'UI recovery model', limit: { context: 32000, output: 1000 } },
        },
      },
      ...
      missing: {
        ...
        models: {
          broken: { name: 'UI preflight failure', limit: { context: 32000, output: 1000 } },
        },
      },
    },
  })
...
await writeFile(
  ...
  JSON.stringify({
    'varro.server.command': resolve(binary),
...
code.stdout.on('data', (data) => {
  codeLog = (codeLog + data).slice(-20000);
});
...
const ownsDatabase = (pid) =>
  spawnSync('lsof', ['-t', '-a', '-p', String(pid), database], { encoding: 'utf8' })
    .stdout.trim()
    .split(/\s+/)
    .includes(String(pid));
...
let outcome;
try {
  await writeVscodeLaunchMetadata(join(root, 'launch.json'), {
    pid: code.pid,
    executable,
    profileRoot: profile,
    userDataDir: join(profile, 'user'),
    extensionsDir: join(profile, 'extensions'),
    workspace,
  ...
  const owner = spawnSync('lsof', ['-t', '-a', '-p', String(lease.pid), database], {
    encoding: 'utf8',
  });
  assert.ok(
    owner.stdout.trim().split(/\s+/).includes(String(lease.pid)),
    'The fixture listener does not hold the isolated database'
  );
  ...
  void (async () => {
    ...
    if (!stream.signal.aborted) errors.push(error.message);
  });
  ...
  await expect(frame.locator('.chat-turn-assistant')).toContainText('UI adapter reply.', {
    timeout: 30000,
  });
  await expect(frame.locator('.chat-turn-assistant').last()).toBeVisible();
  await frame.page().screenshot({ path: join(root, 'healthy.png') });
  await frame
    .locator('.chat-header-session-title')
    .filter({ visible: true })
    .first()
    .click({ button: 'right' });
  await expect(frame.getByRole('menu', { name: 'Session actions' })).toBeVisible();
  await expect(frame.getByRole('menuitem', { name: /^(?:Unshare|Share) session$/ })).toHaveCount(0);
  await frame.page().screenshot({ path: join(root, 'unsupported-actions-hidden.png') });
  await frame.page().keyboard.press('Escape');

  const approvalSession = (await api('GET', '/api/session')).data[0];
  assert.ok(approvalSession?.id);
  await api('PATCH', `/api/session/${approvalSession.id}`, {
    permissions: [{ action: '*', resource: '*', effect: 'ask' }],
  });
  for (const action of ['Allow once', 'Reject']) {
    const pending = fetch(`${serverUrl}/api/session/${approvalSession.id}/permission`, {
      method: 'POST',
      headers: { Authorization: authorization, 'content-type': 'application/json' },
      body: JSON.stringify({
        action: 'shell',
        resources: [`fixture ${action}`],
        save: [],
        metadata: {},
      }),
      signal: AbortSignal.timeout(30000),
    }).then(async (response) => ({ status: response.status, body: await response.json() }));
    await expect(frame.locator('.permission-prompt')).toBeVisible({ timeout: 10000 });
    await frame.getByLabel('Back to sessions').first().click();
    await frame.locator('.session-item').first().click();
    await expect(frame.locator('.permission-prompt')).toBeVisible();
    await frame.getByRole('button', { name: action, exact: true }).click();
    await pending;
    await expect(frame.locator('.permission-prompt')).toHaveCount(0);
    assert.equal((await api('GET', '/api/permission/request')).data.length, 0);
  }
  const question = (
    await api('POST', `/api/session/${approvalSession.id}/form`, {
      title: 'Deep parity choice',
      fields: [
        {
          key: 'choice',
          type: 'string',
  ...
  await expect(frame.getByRole('radio', { name: /Canary/ })).toBeVisible();
  await frame.getByLabel('Back to sessions').first().click();
  await frame.locator('.session-item').first().click();
  await frame.getByRole('radio', { name: /Canary/ }).click();
  await frame.getByRole('button', { name: 'Submit', exact: true }).click();
  await expect(frame.getByRole('radio', { name: /Canary/ })).toHaveCount(0);
  assert.equal((await api('GET', '/api/form')).data.length, 0);
  const answered = (await api('GET', `/api/session/${approvalSession.id}/form/${question.id}`))
    .data;
  assert.equal(answered.state.status, 'answered');
  assert.equal(answered.state.answer.choice, 'canary');
  await api('POST', `/api/session/${approvalSession.id}/form`, {
    title: 'Deep parity cancel',
    fields: [{ key: 'answer', type: 'string', title: 'Optional detail' }],
  });
  await expect(frame.getByRole('button', { name: 'Skip', exact: true })).toBeVisible();
  await frame.getByRole('button', { name: 'Skip', exact: true }).click();
  await expect(frame.getByRole('button', { name: 'Skip', exact: true })).toHaveCount(0);
  assert.equal((await api('GET', '/api/form')).data.length, 0);
  await frame.page().screenshot({ path: join(root, 'approvals-settled.png') });
  await frame.locator('.model-picker-btn').click();
  ...
  await composer.press('Enter');
  await expect(frame.locator('.assistant-message-flow-item-error').last()).toContainText(
    'Re-authenticate',
    { timeout: 30000 }
  );
  await expect(
    frame
      .locator('.assistant-message-flow-item-error')
      .last()
      .getByRole('button', { name: 'Re-authenticate', exact: true })
  ).toBeVisible();
  ...
  await composer.press('Enter');
  await expect(frame.locator('.chat-turn-assistant').last()).toContainText('UI adapter reply.', {
    timeout: 30000,
  });
  await expect(frame.locator('.chat-turn-assistant').last()).toBeVisible();
  ...
  if (await frame.getByLabel('Expand Recycle Bin', { exact: true }).isVisible())
    await frame.getByLabel('Expand Recycle Bin', { exact: true }).click();
  ...
  if (await frame.getByLabel('Expand Recycle Bin', { exact: true }).isVisible())
    await frame.getByLabel('Expand Recycle Bin', { exact: true }).click();
  await frame
    .locator('.recycle-bin-item')
    .getByLabel('Delete permanently', { exact: true })
    .click();
  ...
  const unavailable = (
    await api('POST', '/api/session', {
      title: 'Unavailable backend fixture',
      location: { directory: workspace },
    })
  ).data;
  sessions.add(unavailable.id);
  const unavailableRow = frame
    .locator('.session-item')
    .filter({ hasText: 'Unavailable backend fixture' });
  ...
  await expect(
    frame.getByText('This conversation is unavailable on the connected OpenCode server.', {
      exact: false,
    })
  ).toBeVisible();
  ...
  const seedMessage = (id, sessionID, role, text, at) => {
    ...
    legacy
      ...
      .run(id, sessionID, at, JSON.stringify(info));
    legacy
      .prepare('INSERT INTO part VALUES (?,?,?,?,?)')
      .run('part_' + id, sessionID, id, at, JSON.stringify({ type: 'text', text }));
  };
  legacy
    ...
    .run(
      ...
      1,
      4,
      JSON.stringify({ providerID: 'fixture', modelID: 'fixture' })
    );
  legacy
    .prepare('INSERT INTO session VALUES (?,?,?,?,?,?,?)')
    .run('ses_legacy_child', 'ses_legacy_fixture', workspace, 'Legacy fixture child', 2, 3, null);
  ...
  legacy.prepare('INSERT INTO part VALUES (?,?,?,?,?)').run(
    'part_tool_fixture',
    'ses_legacy_fixture',
    ...
    JSON.stringify({
      ...
      state: {
        status: 'completed',
        input: { prompt: 'Inspect a fixture.' },
        output: 'Child fixture result.',
  ...
  await frame
    ...
    .getByText('Legacy fixture root', { exact: true })
    .click();
  await expect(frame.locator('.chat-turn-assistant')).toContainText('Imported v1 answer.', {
    timeout: 30000,
  });
  assert.equal(
    requests.length,
    beforeProviderRequests,
    'Import must not execute historical tools or start a model turn'
  );
  ...
  await frame
    ...
    .first()
    .fill('Continue the imported conversation.');
  ...
  assert.equal(
    legacySnapshot(),
    beforeImport,
    'Import and continuation changed the original v1 history'
  );
  legacy.close();
  ...
  await frame
    .locator('.session-item')
    .filter({ hasText: 'Legacy fixture root (v1 copy)' })
    .first()
    .click();
  await expect(frame.locator('.chat-turn-assistant')).toContainText([
    'Imported v1 answer.',
    'UI adapter reply.',
  ]);
  ...
  assert.ok(
    spawnSync('lsof', ['-t', '-a', '-p', String(importedLease.pid), database], { encoding: 'utf8' })
      .stdout.trim()
      .split(/\s+/)
      .includes(String(importedLease.pid))
  );
    ...
    await writeFile(
      ...
      JSON.stringify(
        browser
          ...
          .map((page) => ({
            url: page.url(),
            frames: page.frames().map((candidate) => candidate.url()),
          })),
        null,
        2
      )
  ...
  try {
    ...
    if (error.code !== 'ENOENT') errors.push(`Could not read fixture lease: ${error.message}`);
  }
  if (authorization) {
    const inventory = await api('GET', '/api/session').catch(() => ({ data: [] }));
    for (const session of inventory.data) {
      if (
        ['ses_legacy_fixture', 'ses_legacy_child'].includes(
          session.metadata?.varroLegacyImport?.sourceSessionID
        )
      )
  ...
  for (const sessionID of sessions) {
    try {
      ...
      errors.push(`Cleanup ${sessionID}: ${error.message}`);
    }
  }
  if (authorization) {
    try {
      ...
      if (remaining.data.length)
        errors.push(
          `Fixture sessions remain: ${remaining.data.map((session) => session.id).join(', ')}`
        );
    } catch (error) {
      errors.push(`Could not verify fixture cleanup: ${error.message}`);
    }
  }
  await browser?.close();
  code.kill('SIGTERM');
  ...
  await writeFile(
    ...
    JSON.stringify({ ...outcome, database, requests, errors }, null, 2)
  );
