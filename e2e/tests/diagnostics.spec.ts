...
test('About diagnostics preview follows path selection and waits for export acknowledgement', async ({
  page,
}) => {
  ...
  const html = renderAboutHtml(
    {
      ...
      diagnostics: diagnosticTimeline.export(snapshot),
      diagnosticsWithPaths: diagnosticTimeline.export(snapshot, false),
    },
    'http://127.0.0.1:4174'
  );
  ...
  await expect(page.locator('body')).toHaveAttribute(
    ...
    JSON.stringify({ action: 'copyDiagnostics', includePaths: true })
  );
  ...
  await expect(page.locator('body')).toHaveAttribute(
    'data-diagnostic-action',
    JSON.stringify({ action: 'saveDiagnostics', includePaths: false })
  );
});
